...
aws --endpoint-url=http://localhost:4566 sqs set-queue-attributes --queue-url "http://localhost:4566/000000000000/trade_imports_btms_activity_reporting_api" --attributes '{"RedrivePolicy": "{\"deadLetterTargetArn\":\"arn:aws:sqs:eu-west-2:000000000000:trade_imports_btms_activity_reporting_api-deadletter\",\"maxReceiveCount\":\"1\"}"}'
...
function is_ready() {
    ...
    aws --endpoint-url=http://localhost:4566 sqs get-queue-url --queue-name trade_imports_btms_activity_reporting_api-deadletter || return 1
    return 0
}
