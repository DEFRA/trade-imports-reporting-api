...
public class GetMatchesSummaryByLevelTests(ApiWebApplicationFactory factory, ITestOutputHelper outputHelper)
    : EndpointTestBase(factory, outputHelper)
{
    ...
    public async Task Get_WhenUnauthorized_ShouldBeUnauthorized()
    {
        ...
        var response = await client.GetAsync(Testing.Endpoints.Matches.SummaryByLevel());
    ...
    public async Task Get_WhenAuthorized_ShouldBeOk()
    {
        ...
        MockReportRepository
            .GetMatchesSummaryByLevel(from, to, Arg.Any<CancellationToken>())
            .Returns(new MatchesSummaryByLevel(81, 80, 74, 65));

        var response = await client.GetAsync(
            Testing.Endpoints.Matches.SummaryByLevel(
